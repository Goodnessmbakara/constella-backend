...
import uuid
from db.milvus.milvus_client import get_client
...
def insert_record(tenant_name: str, record: GeneralWeaviateRecord):
    ...
    """
    try:
        milvus_dict = process_record_for_insert(
            record.to_milvus_dict(tenant_name))
        ...
        if "vector" not in milvus_dict or not milvus_dict["vector"] or len(milvus_dict["vector"]) != our_embedding_dimension:
            milvus_dict["vector"] = generate_vector_for_milvus_record(
                milvus_dict)
        ...
        if not milvus_client:
            print("Warning: Milvus client not available, skipping insert")
...
def upsert_records(tenant_name: str, records: list[GeneralWeaviateRecord], type='note', long_job_id='', last_update_device='', last_update_device_id=''):
    ...
    try:
        ...
        if milvus_data:
            milvus_client = get_client()
            if not milvus_client:
                print("Warning: Milvus client not available, skipping upsert")
                return
...
def update_record_vector(tenant_name: str, unique_id: str, new_vector: List[float], metadata_updates: Dict = None):
    ...
    """
    try:
        update_record_vector_raw(
            tenant_name, unique_id, new_vector, metadata_updates)
    except Exception as e:
...
def delete_record(tenant_name: str, unique_id: str, record_type: str = "note", s3_path: str = None):
    ...
    try:
        milvus_client = get_client()
        if not milvus_client:
            print("Warning: Milvus client not available, skipping delete")
            return
...
def query_by_vector(tenant_name: str, query_vector: List[float], top_k: int = default_query_limit, similarity_setting: float = 0.5, include_vector: bool = False):
    ...
    try:
        ...
        milvus_client = get_client()
        if not milvus_client:
            print("Warning: Milvus client not available, skipping query")
            return {"results": []}
        results = milvus_client.client.search(
            ...
            limit=top_k,
            filter=f'tenantName == "{tenant_name}"',
            output_fields=["*"] if include_vector else ["uniqueid",
                                                        "recordType", "title", "content", "created", "lastModified"]
        )
...
def query_by_keyword(tenant_name: str, keyword: str, top_k: int = default_query_limit, include_metadata: bool = True):
    ...
    try:
        ...
        results = milvus_client.client.query(
            ...
            filter=filter_expr,
            limit=top_k,
            output_fields=["*"] if include_metadata else ["uniqueid",
                                                          "recordType", "title", "content"]
        )
...
def query_by_filter(tenant_name: str, filter_expr: str, top_k: int = default_query_limit, get_connected_results: bool = False, offset: int = 0):
    ...
    try:
        ...
        if get_connected_results:
            connected = handle_milvus_connected_results(
                tenant_name, results).get("results", [])
...
def get_most_recent_records(tenant_name: str, limit: int):
    ...
    try:
        ...
        candidate_records.sort(key=lambda r: r.get(
            "lastModified", 0), reverse=True)
...
def sync_by_last_modified(tenant_name: str, last_sync_datetime: datetime, curr_device_id: str, limit: int = None, offset: int = None):
    ...
    try:
        ...
        if not (limit is not None and offset is not None and offset > 0):
            try:
                deleted_records = DeletedRecord.get_records_since(
                    tenant_name, last_sync_datetime)
            except (ServerSelectionTimeoutError, AutoReconnect, ConnectionFailure) as mongo_error:
                print(
                    f'MongoDB connection error in deleted records (replica set issue): {mongo_error}')
                print(
                    'Attempting to continue sync without deleted records for this iteration')
                # Try to handle the connection with backoff
                for retry_attempt in range(3):
                    try:
                        delay = (2 ** retry_attempt)  # 1s, 2s, 4s
                        print(
                            f'Waiting {delay} seconds before retry attempt {retry_attempt + 1}/3')
                        time.sleep(delay)
                        ...
                        print(
                            f'MongoDB connection restored on attempt {retry_attempt + 1}')

                        # Retry the deleted records fetch
                        deleted_records = DeletedRecord.get_records_since(
                            tenant_name, last_sync_datetime)
                        print(
                            'Successfully retrieved deleted records after connection recovery')
                        break

                    except Exception as retry_error:
                        if retry_attempt == 2:  # Last attempt
                            print(
                                f'All MongoDB retry attempts failed: {retry_error}')
                            print(
                                'Continuing sync without deleted records - they will be included in next sync')
                            deleted_records = []
                        continue
            except Exception as e:
            ...
            for key, val in filter_input.items():
                if isinstance(val, list):
                    ...
                    if key.lower().endswith("ids") or key == "tagIds":
                        parts.append(
                            f"ARRAY_CONTAINS_ANY({key}, {json.dumps(val)})")
                    else:
...
def handle_milvus_connected_results(tenant_name: str, results: List[Dict]):
    ...
    connection_map = {}
    if connection_ids:
        connected_records = get_records_by_ids(
            tenant_name, list(connection_ids))
...
def fetch_objects_with_fallback(filter_expr: str, output_fields: Optional[List[str]] = None, *, limit: int = 1000, offset: int = 0, smaller_limit: Optional[int] = None) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        print(
            f"Error fetching Milvus objects with limit {limit}, falling back to smaller batches: {e}")
        traceback.print_exc()

    # Fallback to smaller batches
    smaller_limit = smaller_limit or max(limit // 10, 1)
    all_results: List[Dict[str, Any]] = []
    num_batches = (limit + smaller_limit -
                   1) // smaller_limit  # ceiling division
    ...
    for i in range(num_batches):
        ...
        try:
            ...
            print(
                f"Error fetching Milvus batch at offset {batch_offset}: {batch_error}")
            traceback.print_exc()
...
def process_milvus_records(results: List[Dict[str, Any]]):
    ...
    for i, res in enumerate(results or []):
        ...
        for key, value in list(res.items()):
            converted_value = convert_numpy_recursive(
                value, f"record[{i}].{key}")
